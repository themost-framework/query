...
class ClosureParser {
    ...
    parseFilter(func, params) {
        ...
        if (fnExpr.body.type === ExpressionTypes.BinaryExpression || fnExpr.body.type === ExpressionTypes.LogicalExpression) {
    ...
    parseCommon(expr) {
        ...
        else if (expr.type === ExpressionTypes.UnaryExpression) {
    ...
    parseUnary(expr) {
        if (expr.operator === '!') {
            // add an exception about !0 and !1 expression which are generated by typescript compiler
            // and convert them to boolean literals
            if (expr.argument.type === ExpressionTypes.Literal) {
                if (expr.argument.value === 0) {
                    return new LiteralExpression(true);
                }
                else if (expr.argument.value === 1) {
                    return new LiteralExpression(false);
                }
            }
            return new MethodCallExpression('not', [
                this.parseCommon(expr.argument)
            ]);
        }
        throw new Error('Unsupported unary expression operator: ' + expr.operator);
    }
    /**
