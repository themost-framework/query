...
/**
 * Represents the PostgreSQL SQL dialect formatter.
 * @class PostgreSQLDialect
 * @augments {SqlFormatter}
 */
class PostgreSQLDialect extends SqlFormatter {
    ...
    constructor() {
        super();
        this.settings = {
            nameFormat: PostgreSQLDialect.NAME_FORMAT,
            forceAlias: true
        };
    }
    ...
    formatLimitSelect(obj) {
        ...
        if (obj.$take) {
            ...
            if (obj.$skip) {
                sql = sql.concat(' OFFSET ', obj.$skip.toString());
            }
        }
        return sql;
    }
     ...
     */
    $indexof(p0, p1) {
        return sprintf('(STRPOS(%s,%s)-1)', this.escape(p0), this.escape(p1));
    }
     ...
     */
    $text(p0, p1) {
        return sprintf('(STRPOS(%s,%s)-1)>=0', this.escape(p0), this.escape(p1));
    }

    /**
     * Implements simple regular expression formatter using PostgreSQL's ~ operator.
    ...
    $toString(expr) {
        return sprintf('CAST(%s AS TEXT)', this.escape(expr));
    }
    ...
    $toDouble(expr) {
        return sprintf('CAST(%s AS FLOAT)', this.escape(expr));
    }
    ...
    $toBoolean(expr) {
        return sprintf('CAST(%s AS BOOLEAN)', this.escape(expr));
    }
    ...
    $toDate(expr, type) {
        switch (type) {
            ...
            case 'timestamp':
                return sprintf('CAST(%s AS TIMESTAMPTZ)', this.escape(expr));
            default:
                return sprintf('CAST(%s AS DATE)', this.escape(expr));
        }
    }
